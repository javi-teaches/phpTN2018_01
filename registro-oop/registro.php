<?php
	...
	$paises = ['Argentina', 'Bolivia', 'Brasil', 'Colombia', 'Chile', 'Ecuador', 'Paraguay', 'Peru', 'Uruguay', 'Venezuela'];
	...
	if ($_POST) {
		...
		// Si el array de errorres está vacío, es porque no hubo errores,
		// por lo tanto procedo con lo siguiente
		if (empty($errores)) {
			...
			if (empty($errores)) {
				...
				$usuario = new Usuario($_POST["name"], $_POST["email"], $_POST["pass"], $_POST["country"], $picture);
   ...
   <body>
		...
		<div class="data-form">
			<form  method="post" enctype="multipart/form-data">
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group <?= isset($errores['name']) ? 'has-error' : null ?>">
							...
							<input type="text" class="form-control" name="name" value="<?=$name?>" placeholder="<?= isset($errores['name']) ? $errores['name'] : ''; ?>">
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group <?= isset($errores['country']) ? 'has-error' : null ?>">
							<label class="control-label">País:</label>
		               <select class="form-control" class="" name="country">
								...
								<span class="help-block" style="<?= !isset($errores['country']) ? 'display: none;' : ''; ?>">
								<b class="glyphicon glyphicon-exclamation-sign"></b>
								<?= isset($errores['country']) ? $errores['country'] : ''; ?>
							</span>
		            </div>
					</div>
					<div class="col-xs-6">
				...
				<div class="row">
					...
					<div class="col-xs-12">
						<br>
						<a class="btn btn-lg btn-success" href="login.php">Ingresá</a>
					</div>
				</div>
        </form>
	  	</div>
